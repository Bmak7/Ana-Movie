...
class DaddyLiveSource(private val context: Context) {
    ...
    suspend fun fetchLiveStreamLink(channelJson: String): List<Video>? = withContext(Dispatchers.IO) {
        try {
            ...
            val streamIdMatcher = streamIdPattern.matcher("$baseUrl/watch.php?id=${channel.id}")
            val streamId = if (streamIdMatcher.find()) {
                streamIdMatcher.group(1) ?: run {
                    Log.e("DDL", "Could not extract stream ID from URL: $baseUrl/watch.php?id=${channel.id}")
                    return@withContext null
                }
            } else {
                Log.e("DDL", "Invalid URL format: $baseUrl/watch.php?id=${channel.id}")
                return@withContext null
            }
            ...
            val watchVideos = tryExtractFromWatchPage(watchUrl, streamId)
            if (!watchVideos.isNullOrEmpty()) {
                Log.d("DDL", "Successfully got ${watchVideos.size} stream(s) from watch page")
                videoList.addAll(watchVideos)
            }
            ...
            for (endpoint in playerEndpoints) {
                ...
                val videosFromEndpoint = tryFetchFromPlayerEndpoint(playerUrl, channel.name, streamId)
                if (!videosFromEndpoint.isNullOrEmpty()) {
                    Log.d("DDL", "Successfully got ${videosFromEndpoint.size} stream(s) from endpoint: $endpoint")
                    videoList.addAll(videosFromEndpoint)
                }
            }

            if (videoList.isNotEmpty()) {
                val distinctVideos = videoList.distinctBy { it.url }
                Log.d("DDL", "Found a total of ${distinctVideos.size} distinct streams for channel: ${channel.name}")
    ...
    private suspend fun tryExtractFromWatchPage(watchUrl: String, streamId: String): List<Video>? {
        try {
            Log.d("DDL", "Trying watch page: $watchUrl")
            val request = Request.Builder().url(watchUrl).addHeader("User-Agent", userAgent).addHeader("Referer", baseUrl).build()
...
//            logLongString("DDL_HTML", "Watch Page HTML:\n$content")
            ...
            while (matcher.find()) {
                ...
                val fullPlayerUrl = if (playerPath.startsWith("http")) playerPath else "$baseUrl$playerPath"
                Log.d("DDL", "Trying player URL from watch page: $fullPlayerUrl")
                val videosFromEndpoint = tryFetchFromPlayerEndpoint(fullPlayerUrl, "", streamId)
                if (!videosFromEndpoint.isNullOrEmpty()) {
                    videos.addAll(videosFromEndpoint)
                }
            }
            return if (videos.isNotEmpty()) videos else null
        } catch (e: Exception) {
    ...
    private suspend fun tryFetchFromPlayerEndpoint(playerUrl: String, channelName: String, streamId: String): List<Video>? {
        try {
            Log.d("DDL", "Trying to fetch from URL: $playerUrl")
            val request = Request.Builder().url(playerUrl).addHeader("User-Agent", userAgent).addHeader("Referer", baseUrl).addHeader("Origin", baseUrl).build()
            val response = client.newCall(request).execute()
...
//            logLongString("DDL_HTML", "Player Page HTML from $playerUrl:\n$pageContent")
            ...
            for (pattern in iframePatterns) {
                ...
                while (matcher.find()) {
                    val iframeSrc = matcher.group(1) ?: continue
                    ...
                    val fullIframeUrl = if (iframeSrc.startsWith("http")) {
                        ...
                        playerUrl.substringBeforeLast("/") + "/" + iframeSrc
                    }
    ...
    private suspend fun extractFinalUrl(iframeSrc: String, referer: String): String? {
        try {
            ...
            val iframeRequest = Request.Builder().url(iframeSrc).addHeader("User-Agent", userAgent).addHeader("Referer", referer).addHeader("Origin", referer.substringBeforeLast("/")).build()
...
//            logLongString("DDL_JS", "Iframe content from $iframeSrc:\n$scriptContent")
            ...
            if (iframeSrc.contains("lovecdn.ru")) {
                val nestedIframePattern = Pattern.compile("""<iframe[^>]*src=["']([^"']+)["']""")
                val nestedMatcher = nestedIframePattern.matcher(scriptContent)
                if (nestedMatcher.find()) {
                    val nestedIframeSrc = nestedMatcher.group(1) ?: return null
                    Log.d("DDL", "Found nested iframe in lovecdn: $nestedIframeSrc")

                    // START: FIX - Correctly extract stream name and token for Flussonic player
                    if (nestedIframeSrc.contains("/embed.html")) {
                        try {
                            Log.d("DDL", "Found Flussonic embed, attempting direct extraction.")
                            val planetaryRequest = Request.Builder().url(nestedIframeSrc).addHeader("User-Agent", userAgent).addHeader("Referer", iframeSrc).build()
                            val planetaryResponse = client.newCall(planetaryRequest).execute()
                            val planetaryContent = planetaryResponse.body?.string() ?: ""
                            planetaryResponse.close()

                            val streamNamePattern = Pattern.compile(""""streamName"\s*:\s*"([^"]+)"""")
                            val streamNameMatcher = streamNamePattern.matcher(planetaryContent)
                            if (streamNameMatcher.find()) {
                                val streamName = streamNameMatcher.group(1)
                                val planetaryUrl = URL(nestedIframeSrc)

                                // Extract the token from the iframe URL's query string
                                val tokenQuery = planetaryUrl.query

                                // Construct the final M3U8 URL with the token
                                val finalUrl = "${planetaryUrl.protocol}://${planetaryUrl.host}/$streamName/index.m3u8" +
                                        if (!tokenQuery.isNullOrEmpty()) "?$tokenQuery" else ""

                                Log.d("DDL", "Constructed Flussonic M3U8 URL with token: $finalUrl")
                                return finalUrl
                            }
                        } catch (e: Exception) {
                            Log.e("DDL", "Flussonic direct extraction failed: ${e.message}")
                        }
                    }
                    // END: FIX

                    // Fallback to recursive call for other nested iframes
                    return extractFinalUrl(nestedIframeSrc, iframeSrc)
                }
            }

            // START: FIX 2 - Make external JS fetching more resilient
            ...
            while (scriptMatcher.find()) {
                ...
                val fullScriptUrl = if (scriptUrl.startsWith("http")) scriptUrl else if (scriptUrl.startsWith("//")) "https:$scriptUrl" else if (scriptUrl.startsWith("/")) "${URL(iframeSrc).protocol}://${URL(iframeSrc).host}$scriptUrl" else iframeSrc.substringBeforeLast("/") + "/" + scriptUrl
                ...
                try {
                    ...
                    // We don't need the content, just making the request can be necessary
                    client.newCall(jsRequest).execute().close()
                } catch (e: Exception) {
                    // Log the failure but don't crash the extraction process
                    Log.w("DDL", "Failed to fetch non-essential external JS: $fullScriptUrl")
                }
            }
            // END: FIX 2
            ...
            val serverUrl = when {
                ...
                else -> { val parsedUrl = URL(iframeSrc); "${parsedUrl.protocol}://${parsedUrl.host}" }
            }
            ...
            val bundlePatterns = listOf(
                ...
                Pattern.compile("""const\s+XKZK\s*=\s*["']([^"']+)["']"""),
            ...
            if (channelKey == null) {
                val urlPatterns = listOf(Pattern.compile("""[?&]id=(\d+)"""), Pattern.compile("""[?&]a=(\d+)"""), Pattern.compile("""[?&]channel=(\d+)"""))
...
//                logLongString("DDL_JS_FAILED", "Full script content for debugging: $scriptContent")
            ...
            val bundleJson = try { String(Base64.decode(bundle, Base64.DEFAULT)) } catch (e: Exception) { Log.e("DDL", "Failed to decode bundle: ${e.message}"); return null }
            Log.d("DDL", "Decoded bundle JSON: $bundleJson")
            val bundleObj = try { json.decodeFromString<Bundle>(bundleJson) } catch (e: Exception) { Log.e("DDL", "Failed to parse bundle JSON: ${e.message}"); return null }

            val authTs = base64Decode(bundleObj.bTs)
            val authRnd = base64Decode(bundleObj.bRnd)
            val authSig = base64Decode(bundleObj.bSig)
            if (authTs.isEmpty() || authRnd.isEmpty() || authSig.isEmpty()) { Log.e("DDL", "Failed to decode auth parameters"); return null }
            ...
            val authRequest = Request.Builder().url(authUrl).addHeader("User-Agent", userAgent).addHeader("Referer", "$serverUrl/").addHeader("Origin", serverUrl).build()
            val authResponse = client.newCall(authRequest).execute()
            Log.d("DDL", "Auth response code: ${authResponse.code}")
            if (authResponse.code == 403) { Log.e("DDL", "Auth request failed with 403 Forbidden"); authResponse.close(); return null }
            authResponse.close()

            val serverLookupUrl = "$serverUrl/server_lookup.php?channel_id=$channelKey"
            Log.d("DDL", "Server lookup URL: $serverLookupUrl")
            val serverKeyRequest = Request.Builder().url(serverLookupUrl).addHeader("User-Agent", userAgent).addHeader("Referer", "$serverUrl/").addHeader("Origin", serverUrl).build()
            val serverKeyResponse = client.newCall(serverKeyRequest).execute()
            val serverKeyJson = serverKeyResponse.body?.string() ?: run { Log.e("DDL", "Empty response from server lookup"); serverKeyResponse.close(); return null }
            Log.d("DDL", "Server lookup response code: ${serverKeyResponse.code}, content: $serverKeyJson")
            serverKeyResponse.close()
            val serverData = try { json.decodeFromString<ServerKeyResponse>(serverKeyJson) } catch (e: Exception) { Log.e("DDL", "Failed to parse server key JSON: ${e.message}"); return null }
            val serverKey = serverData.serverKey
            Log.d("DDL", "Extracted server key: $serverKey")

            val finalUrl = if (serverKey == "top1/cdn") "https://top1.newkso.ru/top1/cdn/$channelKey/mono.m3u8" else "https://$serverKey.newkso.ru/$serverKey/$channelKey/mono.m3u8"
...
//                Pattern.compile("""const\s+BUNDLE\s*=\s*["']([^"']+)["']"""),
//                Pattern.compile("""const\s+XJZ\s*=\s*["']([^"']+)["']"""),
//                // CORRECTION: Add the new variable name 'XKZK' to the patterns
//                Pattern.compile("""const\s+XKZK\s*=\s*["']([^"']+)["']"""),
//                Pattern.compile("""const\s+\w+\s*=\s*["']([^"']+)["']"""),
//                Pattern.compile("""var\s+\w+\s*=\s*["']([^"']+)["']""")
//            )
